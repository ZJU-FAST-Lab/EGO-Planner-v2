...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    for (int i = 0; i < drone_num; ++i)
    {
      ...
      // release goal
      if ( drones_[i].goal_to_release_id >= 0 )
      {
        if ( (drones_[i].cur_p - goals[drones_[i].goal_to_release_id].p).norm() > 0.5 ) // 0.5m threshold
        {
          goals[drones_[i].goal_to_release_id].occupied = false;
          cout << ros::Time::now() << " RELEASE GOAL:" << " drone_id=" << i << " goal=" << goals[drones_[i].goal_to_release_id].p.transpose() << endl;
          drones_[i].goal_to_release_id = -1;
        }
      }
    }
    ...
    while (drone_trials < drone_num)
    {
      ...
      if (drones_[d_id].odom_received && drones_[d_id].arrived_for_a_while)
      {
        ...
        while (goal_trials < goal_num)
        {
          ...
          {
            // prepare for releasing arrived goals
            if (drones_[d_id].goal_id >= 0)
            {
              drones_[d_id].goal_to_release_id = drones_[d_id].goal_id;
            }
            ...
            cout << ros::Time::now() << " SET GOAL:" << " drone_id=" << d_id << " goal=" << drones_[d_id].goal.transpose() << endl;
            goto multi_loop;
          }
